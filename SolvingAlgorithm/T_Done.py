...
class cube_state:
	...
	def apply_move(self, move):
		...
		for turn in range(turns):
			...
			for i in range(8):
				isCorner = int(i > 3)
				target = affected_cubies[int(face)][i] + isCorner*12
				killer = affected_cubies[int(face)][(i-3) if (i&3)==3 else i+1] + isCorner*12
...
def solve_cube(cube_s):
	...
	state2 = [0] * 40
	state2 = cube_state(cube.get_orientataion())
	...
	move_sequence = solve_Thistlethwaite(state2,goal_state)
