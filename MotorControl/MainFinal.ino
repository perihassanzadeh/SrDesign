...
int RPM = 600;  //  set RPM for cruise speed
int ACCEL = 8000;  // Acceleration and deceleration values are always in FULL steps / s^2
int DECEL = 8000;
...
A4988 motor6(MOTOR_STEPS, DIR, STEP); //a = 110
...
String moves[60]; //all moves read from serial
int movesLength = 0;
  ...
  switch(stateLoop)
  {
    ...
    case Shuffle://**/////////////////////////////////////////////////////////////////////////////////////////**
      ...
      if(B2Flag){ //button to start shuffling
        ...
        inputSequence = "";
        memset(moves, "", sizeof(moves)); //clear moves
        state = Home;
        B2Flag = 0;
      }
...
/*
        ...
        */
        inputSequence = "";
        Serial.setTimeout(30000); //set timeout to 30 sec
        inputSequence = Serial.readString();   //read until timeout
        inputSequence.trim(); 
...
*/
    ...
    case Solve://**/////////////////////////////////////////////////////////////////////////////////////////**
      ...
      if(solveType){ //if step-solve
        if(CWFlag){  //if encoder rotates clockwise - go to next move                                 //currentPosition:   0   1   2   3   4   5   6
          if(currentPosition < movesLength){       //if theres a next step                            //moves[]:           | 0 | 1 | 2 | 3 | 4 | 5 |          --> movesLength = 6
            lcd.setCursor(6, 1);
            lcd.print(moves[currentPosition]);
            lcd.print(" ");
            lcd.setCursor(10, 1);
            ...
            lcd.print(movesLength);
            lcd.print("  ");
            rotateMotor(moves[currentPosition]);   //rotate motor with next move
            currentPosition++;                     //increment currentPosition
          }
          
          if(currentPosition == movesLength)  //if at end of sequence, indicate to go home
            blinkYellow(-1);
        ...
        if(CCWFlag){ //if encoder rotates counter-clockwise - go back to last move
          if(currentPosition > 0){       //if theres a previous step
            lcd.setCursor(6, 1);
            lcd.print(moves[currentPosition-1]);
            lcd.print(" ");
            lcd.setCursor(10, 1);
            ...
            lcd.print(movesLength);
            lcd.print("  ");
            reverseRotateMotor(moves[currentPosition-1]);   //rotate motor opposite of last move
        ...
        if(B1Flag){ //home button goes home and clears all scan variables
          ...
          inputSequence = ""; //clear input string and move array
          memset(moves, "", sizeof(moves)); //clear moves[]
          state = Home;
          B1Flag = 0;
        }
        ...
        if(B2Flag){ //button to start solve, run motor sequence
          ...
          for(int i=0; i<movesLength; i++){
            ...
            lcd.print(moves[i]);
            lcd.print(" ");
            rotateMotor(moves[i]);  //rotate motor
          }
...
void rotateMotor90(int numMotor, bool direction)
{
  switch(numMotor)
  {
    case 0:
      ...
      digitalWrite(a2, 0);
      if(direction){
        motor0.rotate(-99);  //backlash compensation
        motor0.rotate(9);
      }
      else{
        motor0.rotate(99);
...
void rotateMotor180(int numMotor)  //direction doesn't matter for 180
{
  switch(numMotor)
  {
    case 0:
      ...
      digitalWrite(a1, 0);
      digitalWrite(a2, 0);
      motor0.rotate(189);  //backlash compensation
      motor0.rotate(-9);
      break;
    case 1:
      digitalWrite(a0, 1);
